...
To do so, ***I will refactor this application into a microservice architecture using message passing techniques that you have learned in this course***. It’s easy to get lost in the countless optimizations and changes that can be made: my priority is to approach the task as an architect and refactor the application into microservices. 
...
4. `kubectl apply -f deployment/udaconnect-api.yaml` - Set up the service and deployment for the  legacy API
5. `kubectl apply -f deployment/udaconnect-connection-microservice.yaml` - Set up the service and deployment for the connection API
6. `kubectl apply -f deployment/udaconnect-person-microservice.yaml` - Set up the service and deployment for the person API
7. `kubectl apply -f deployment/udaconnect-location-producer-microservice.yaml` - Set up the location data producer service 
8. `kubectl apply -f deployment/udaconnect-location-consumer-microservice.yaml` - Set up the location data consumer service
...
11. Install and activate Apache Kafka cluster using Strimzi project. This is done by running the commands in the following order:
 - ```kubectl apply -f deployment/udaconnect-kafka.yaml``` (Create a namespace called kafka)
 - ```kubectl create -f 'https://strimzi.io/install/latest?namespace=kafka' -n kafka``` (Apply the Strimzi install files )
 - ```kubectl apply -f https://strimzi.io/examples/latest/kafka/kafka-persistent-single.yaml -n kafka ``` (Create an Apache Kafka cluster)
 - ```kubectl wait kafka/my-cluster --for=condition=Ready --timeout=300s -n kafka``` (Wait while Kubernetes starts the required pods, services, and so on:)
 - ```kubectl -n kafka run kafka-producer -ti --image=quay.io/strimzi/kafka:0.32.0-kafka-3.3.1 --rm=true --restart=Never -- bin/kafka-console-producer.sh --bootstrap-server my-cluster-kafka-bootstrap:9092 --topic location ``` (With the cluster running, run a simple producer to send messages to a specific Kafka topic name location)
 - ```kubectl -n kafka run kafka-consumer -ti --image=quay.io/strimzi/kafka:0.32.0-kafka-3.3.1 --rm=true --restart=Never -- bin/kafka-console-consumer.sh --bootstrap-server my-cluster-kafka-bootstrap:9092 --topic location --from-beginning``` (To receive the message in a different terminal from the topic name location)
